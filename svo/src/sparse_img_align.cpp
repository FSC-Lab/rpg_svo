...
size_t SparseImgAlign::run(FramePtr ref_frame, FramePtr cur_frame)
{
  ...
  Sophus::SE3d T_cur_from_ref(cur_frame_->T_f_w_ * ref_frame_->T_f_w_.inverse());
    ...
    for(int y=0; y<patch_size_; ++y)
    {
      ...
      for(int x=0; x<patch_size_; ++x, ++pixel_counter, ++cur_img_ptr, ++ref_patch_cache_ptr)
      {
        ...
        if(linearize_system)
        {
          ...
          const Sophus::Vector6d J(jacobian_cache_.col(feature_counter*patch_area_ + pixel_counter));
