    ...
    const double timestamp)
{
  ...
  if(pub_pose_.getNumSubscribers() > 0 && slam.stage() == FrameHandlerBase::STAGE_DEFAULT_FRAME)
  {
    ...
    if(publish_world_in_cam_frame_)
    {
      ...
      Sophus::SE3d T_cam_from_world(frame->T_f_w_* T_world_from_vision_);
      q = T_cam_from_world.unit_quaternion();
    ...
    else
    {
      ...
      Sophus::SE3d T_world_from_cam(T_world_from_vision_*frame->T_f_w_.inverse());
      q = T_world_from_cam.unit_quaternion()*T_world_from_vision_.inverse().unit_quaternion();
...
{
  // publish keyframe
  Sophus::SE3d T_world_cam(T_world_from_vision_*frame->T_f_w_.inverse());
...
void Visualizer::exportToDense(const FramePtr& frame)
{
      ...
      && trace_id_%dense_pub_nth_ == 0 && pub_dense_.getNumSubscribers() > 0)
  {
    ...
    // publish cam in world frame
    Sophus::SE3d T_world_from_cam(T_world_from_vision_*frame->T_f_w_.inverse());
    const Quaterniond& q = T_world_from_cam.unit_quaternion();
